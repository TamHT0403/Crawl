...
export function TikTokAccountManager() {
  ...
  const openEdit = (account: TikTokAccountRow) => {
  ...
  const save = () => {
    ...
    setValidating(true);
    setStatus("🔄 Đang validate session với TikTok...");
    startTransition(async () => {
      try {
        const method = editingId ? "PUT" : "POST";
        const url = editingId
          ? `/api/tiktok/accounts/${editingId}`
          : "/api/tiktok/accounts";
      ...
      {!showForm && accounts.length === 0 && (
        <div className="mt-4 rounded border border-amber-200 bg-amber-50 p-4 text-xs leading-6 text-amber-800">
          <strong className="flex items-center gap-1.5">
            ...
            Cách lấy TikTok Session Cookies:
          </strong>
          <ol className="mt-2 list-decimal space-y-1 pl-4">
            <li>Mở <strong>Microsoft Edge</strong> (hoặc Chrome)</li>
            <li>Đăng nhập <strong>https://www.tiktok.com</strong></li>
            <li>F12 → Tab <strong>Application</strong> (hoặc Cookies)</li>
            <li>Ở mục <strong>Storage</strong> → <strong>Cookies</strong> → Chọn <strong>https://www.tiktok.com</strong></li>
            <li>
            ...
            <li>
              ...
              dán vào <a href="https://www.convertonline.io/convert/cookie-json-to-netscape" target="_blank"
                         className="underline font-semibold">công cụ chuyển đổi</a> → lấy JSON
            </li>
